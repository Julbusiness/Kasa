...
export default function Logement() {
	...
	//! il faut mapper sur les categories numerotation images
	...
	return (
		...
		<div className="container">
			<div className="bloc-carousel">
				<Carousel>
					{elementAPI.pictures.map(picture => <CarouselItem key={picture}><img src={picture} alt="" className="picture"/></CarouselItem>)}
				</Carousel>
			</div>
			...
			<div className="bloc-tag">
				<div className="tags">
					{elementAPI.tags.map(tag => <div 
					key={tag}
					className="tag">{tag}</div>)}
				</div>
				<div className="rating">
					<Rating 
						rating={parseInt(elementAPI.rating)}
					/>
				</div>
			</div>
			<div className="bloc-accordeon">
				...
				<Accord
					title="Equipements"
					content={elementAPI.equipments.map(equipment => <li key={equipment} className="equipment-list">{equipment}</li>)}
					className="accord-logement"
				/>
			</div>
		</div>
